...
export default function Dashboard() {
  const [activeTab, setActiveTab] = useState<
    "energy-dashboard" | "energy-trading" | "simulation"
  >("energy-dashboard");
  ...
  const handleLocationGranted = (location: UserLocation) => {
    ...
    toast({
      ...
      description:
        "Weather data will now be customized for your location to improve solar efficiency calculations.",
    });
  };
  ...
  const handleLocationDenied = () => {
    ...
    toast({
      ...
      description:
        "Using general weather data. You can enable location access later in your browser settings.",
      variant: "destructive",
    });
  };
  ...
  const { data: availableTrades = [], isLoading: availableTradesLoading } =
    useQuery<Array<{ trade: EnergyTrade; household: any; user: any }>>({
  ...
  const { data: userTradeAcceptances = [] } = useQuery<
    Array<{
      id: number;
      tradeId: number;
      status: string;
      acceptorUserId: number;
      acceptorHouseholdId: number;
    }>
  >({
  ...
  const updateTradeMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) =>
      apiRequest("PUT", `/api/energy-trades/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/energy-trades"] });
      queryClient.invalidateQueries({ queryKey: ["/api/trade-offers"] });
  ...
  const deleteTradeMutation = useMutation({
    mutationFn: (id: number) =>
      apiRequest("DELETE", `/api/energy-trades/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/energy-trades"] });
      queryClient.invalidateQueries({ queryKey: ["/api/trade-offers"] });
      toast({
  ...
  const cancelTradeMutation = useMutation({
    mutationFn: (acceptanceId: number) =>
      apiRequest("DELETE", `/api/trade-acceptances/${acceptanceId}`),
    onSuccess: async (data, acceptanceId) => {
      ...
      >(["/api/trade-acceptances"], (prev) =>
        (prev || []).filter((a) => a.id !== acceptanceId),
      );
  ...
  const getHouseholdName = (
    ...
    if (type === "seller") {
      return (
        tradeItem.sellerHouseholdName ||
        (tradeItem.sellerHouseholdId
          ? `Household #${tradeItem.sellerHouseholdId}`
          : "Unknown Household")
      );
    } else {
      return (
        tradeItem.buyerHouseholdName ||
        (tradeItem.buyerHouseholdId
          ? `Household #${tradeItem.buyerHouseholdId}`
          : "Unknown Household")
      );
    }
  };
  ...
  const hasUserApplied = (tradeId: number) => {
    return (userTradeAcceptances || []).some(
      (acceptance) =>
        acceptance.tradeId === tradeId &&
        ["applied", "accepted", "pending", "awarded"].includes(
          acceptance.status,
        ),
    );
  };
  ...
  const getUserAcceptance = (tradeId: number) => {
    return (userTradeAcceptances || []).find(
      (acceptance) =>
        acceptance.tradeId === tradeId &&
        ["applied", "accepted", "pending", "awarded"].includes(
          acceptance.status,
  ...
  const handleCancelApplication = (tradeId: number) => {
    ...
    if (
      acceptance &&
      window.confirm(
        "Are you sure you want to cancel your application for this trade?",
      )
    ) {
  ...
  const acceptTradeMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({
        ...
        description:
          data.message ||
          "You have successfully accepted this trade offer. Contact information will be shared.",
      });
  ...
  const isOwnTrade = (trade: EnergyTrade): boolean => {
    ...
    if (trade.tradeType === "sell") {
      return trade.sellerHouseholdId
        ? userHouseholdIds.includes(trade.sellerHouseholdId)
        : false;
    }

    // For buy trades, check if buyerHouseholdId belongs to user
    if (trade.tradeType === "buy") {
      return trade.buyerHouseholdId
        ? userHouseholdIds.includes(trade.buyerHouseholdId)
        : false;
    }
  ...
  const calculateMarketRates = () => {
    if (Array.isArray(energyTrades) && energyTrades.length > 0) {
      ...
      const activeTrades = (energyTrades as EnergyTrade[]).filter(
        (trade) =>
          trade.status === "pending" &&
          trade.pricePerKwh >= 3 &&
          trade.pricePerKwh <= 20,
      );

      // Separate sell and buy trades
      const sellTrades = activeTrades.filter(
        (trade) => trade.tradeType === "sell",
      );
      const buyTrades = activeTrades.filter(
      ...
      if (sellTrades.length > 0) {
        const totalSellingPrice = sellTrades.reduce(
          (sum, trade) => sum + Math.round(trade.pricePerKwh),
          0,
        );
      ...
      if (buyTrades.length > 0) {
        const totalBuyingPrice = buyTrades.reduce(
          (sum, trade) => sum + Math.round(trade.pricePerKwh),
          0,
        );
  ...
  const tradeFormSchema = z.object({
    ...
    energyAmount: z.coerce
      .number()
      .min(1, "Energy amount must be at least 1 kWh"),
  ...
  const onSubmit = (data: any) => {
    ...
    const tradeData = {
      ...
      sellerHouseholdId:
        data.tradeType === "sell" ? userHouseholdId : undefined,
  ...
  const handleDeleteTrade = (trade: EnergyTrade) => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete this ${trade.tradeType} trade for ${trade.energyAmount} kWh?`,
      )
    ) {
  ...
  const [offerFilter, setOfferFilter] = useState<
    "all" | "cheapest" | "biggest"
  >("all");
  const [requestFilter, setRequestFilter] = useState<
    "all" | "cheapest" | "biggest"
  >("all");
  const [selectedTradeForDetails, setSelectedTradeForDetails] = useState<{
    trade: EnergyTrade;
    household: any;
    user: any;
  } | null>(null);
  ...
  const [acceptingTrade, setAcceptingTrade] = useState<{
    trade: EnergyTrade;
    household: any;
    user: any;
  } | null>(null);
  ...
  let energyOffers = (availableTrades as any[]).filter(
    (item: any) =>
      item.trade.tradeType === "sell" && item.trade.status === "pending",
  );
  let energyRequests = (availableTrades as any[]).filter(
    (item: any) =>
      item.trade.tradeType === "buy" && item.trade.status === "pending",
  );

  // Apply filters
  if (offerFilter === "cheapest") {
    energyOffers = energyOffers.sort(
      (a: any, b: any) => a.trade.pricePerKwh - b.trade.pricePerKwh,
    );
  } else if (offerFilter === "biggest") {
    energyOffers = energyOffers.sort(
      (a: any, b: any) => b.trade.energyAmount - a.trade.energyAmount,
    );
  }

  if (requestFilter === "cheapest") {
    energyRequests = energyRequests.sort(
      (a: any, b: any) => b.trade.pricePerKwh - a.trade.pricePerKwh,
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
              ...
              <motion.div
                ...
                <div className="bg-gradient-to-r from-slate-800/50 via-slate-700/30 to-slate-800/50 p-4 sm:p-6 rounded-xl border border-slate-600/30 backdrop-blur-sm">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
                    <h2 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-blue-300 via-emerald-300 to-purple-300 bg-clip-text text-transparent">
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto border-slate-600 hover:bg-slate-700/50 bg-gradient-to-r from-slate-700/50 to-slate-600/50">
                      <RefreshCw
                ...
                <motion.div
                    ...
                    <Card className="p-3 sm:p-4 bg-gradient-to-br from-emerald-950/40 via-emerald-900/25 to-emerald-800/30 border-emerald-700/40 relative overflow-hidden backdrop-blur-sm hover:scale-105 transition-transform duration-300">
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xs sm:text-sm font-medium text-emerald-400 mb-1 truncate">
                            Network Generation
                          </h3>
                          <p
                            className="text-lg sm:text-2xl font-bold text-emerald-200 truncate"
                            data-testid="text-total-generation">
                            {networkAnalytics?.network
                              ?.totalGenerationCapacity || "0 kW"}
                          </p>
                  ...
                  <motion.div
                    ...
                    <Card className="p-3 sm:p-4 bg-gradient-to-br from-blue-950/40 via-blue-900/25 to-blue-800/30 border-blue-700/40 relative overflow-hidden backdrop-blur-sm hover:scale-105 transition-transform duration-300">
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xs sm:text-sm font-medium text-blue-400 mb-1 truncate">
                            Energy Trades
                          </h3>
                          <p
                            className="text-lg sm:text-2xl font-bold text-blue-200 truncate"
                  ...
                  <motion.div
                    ...
                    <Card className="p-3 sm:p-4 bg-gradient-to-br from-purple-950/40 via-purple-900/25 to-purple-800/30 border-purple-700/40 relative overflow-hidden backdrop-blur-sm hover:scale-105 transition-transform duration-300">
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xs sm:text-sm font-medium text-purple-400 mb-1 truncate">
                            Average Price
                          </h3>
                          <p
                            className="text-lg sm:text-2xl font-bold text-purple-200 truncate"
                            data-testid="text-average-price">
                            {networkAnalytics?.trading?.averagePrice || "₹0"}
                            /kWh
                          </p>
                  ...
                  <motion.div
                    ...
                    <Card className="p-3 sm:p-4 bg-gradient-to-br from-green-950/40 via-emerald-900/25 to-green-800/30 border-green-700/40 relative overflow-hidden backdrop-blur-sm hover:scale-105 transition-transform duration-300">
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xs sm:text-sm font-medium text-green-400 mb-1 truncate">
                            Carbon Saved
                          </h3>
                          <p
                            className="text-lg sm:text-2xl font-bold text-green-200 truncate"
                            data-testid="text-carbon-saved">
                            {networkAnalytics?.trading?.carbonSaved
                              ? formatCarbonSavings(
                                  parseFloat(
                                    networkAnalytics.trading.carbonSaved.replace(
                                      /[^\d.]/g,
                                      "",
                                    ),
                                  ),
                ...
                {marketLoading ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                    <Card className="p-4 sm:p-6 bg-gradient-to-br from-slate-900/60 via-blue-950/40 to-slate-800/60 border border-blue-500/20 backdrop-blur-sm shadow-xl">
                      <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 flex items-center gap-2">
                        ...
                        <span className="hidden sm:inline bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
                          Live Market Activity
                        </span>
                        <span className="sm:hidden bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
                          Market Activity
                        </span>
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        <div className="flex justify-between items-center p-3 sm:p-4 bg-gradient-to-r from-emerald-900/40 to-emerald-800/30 rounded-xl border border-emerald-500/30 hover:border-emerald-400/50 transition-all duration-300 hover:shadow-lg backdrop-blur-sm">
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-emerald-300 truncate">
                              Available for trading
                            </p>
                          </div>
                          <div className="text-right">
                            <p
                              className="text-lg sm:text-xl font-bold text-emerald-300 ml-2"
                              data-testid="text-current-supply">
                              {marketData?.supply ?? "No data"}{" "}
                              {marketData?.supply ? "kWh" : ""}
                            </p>
                          </div>
                        </div>
                        <div className="flex justify-between items-center p-3 sm:p-4 bg-gradient-to-r from-blue-900/40 to-blue-800/30 rounded-xl border border-blue-500/30 hover:border-blue-400/50 transition-all duration-300 hover:shadow-lg backdrop-blur-sm">
                          ...
                          <div className="text-right">
                            <p
                              className="text-lg sm:text-xl font-bold text-blue-300 ml-2"
                              data-testid="text-current-demand">
                              {marketData?.demand ?? "No data"}{" "}
                              {marketData?.demand ? "kWh" : ""}
                            </p>
                          </div>
                        </div>
                        <div className="flex justify-between items-center p-3 sm:p-4 bg-gradient-to-r from-purple-900/40 to-purple-800/30 rounded-xl border border-purple-500/30 hover:border-purple-400/50 transition-all duration-300 hover:shadow-lg backdrop-blur-sm">
                          ...
                          <div className="text-right">
                            <p
                              className="text-lg sm:text-xl font-bold text-purple-300 ml-2"
                              data-testid="text-grid-stability">
                              {marketData?.gridStability ?? "No data"}
                              {marketData?.gridStability ? "%" : ""}
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <Card className="p-4 sm:p-6 bg-gradient-to-br from-sky-950/60 via-blue-900/40 to-indigo-950/60 border border-sky-400/30 backdrop-blur-sm shadow-xl">
                      <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 flex flex-col sm:flex-row sm:items-center gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="hidden sm:inline bg-gradient-to-r from-sky-400 to-blue-300 bg-clip-text text-transparent">
                            Weather Impact
                          </span>
                          <span className="sm:hidden bg-gradient-to-r from-sky-400 to-blue-300 bg-clip-text text-transparent">
                            Weather
                          </span>
                        </div>
                        <span className="bg-green-900/30 text-green-400 px-2 py-1 rounded-full text-xs font-medium">
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        <div className="flex justify-between items-center p-3 sm:p-4 bg-gradient-to-r from-sky-900/40 to-sky-800/30 rounded-xl border border-sky-500/30 hover:border-sky-400/50 transition-all duration-300 hover:shadow-lg backdrop-blur-sm">
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-sky-300 truncate">
                              Weather status
                            </p>
                          </div>
                          <div className="text-right">
                            <p
                              className="text-lg sm:text-xl font-bold text-sky-300 ml-2 capitalize"
                              data-testid="text-weather-condition">
                              {marketData?.weather?.condition?.replace(
                                "-",
                                " ",
                              ) || "No data"}
                            </p>
                        ...
                        <div className="flex justify-between items-center p-3 sm:p-4 bg-gradient-to-r from-orange-900/40 to-orange-800/30 rounded-xl border border-orange-500/30 hover:border-orange-400/50 transition-all duration-300 hover:shadow-lg backdrop-blur-sm">
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-orange-300 truncate">
                              Current temp
                            </p>
                          </div>
                          <div className="text-right">
                            <p
                              className="text-lg sm:text-xl font-bold text-orange-300 ml-2"
                              data-testid="text-temperature">
                              {marketData?.weather?.temperature
                        ...
                        <div className="flex justify-between items-center p-3 sm:p-4 bg-gradient-to-r from-emerald-900/40 to-emerald-800/30 rounded-xl border border-emerald-500/30 hover:border-emerald-400/50 transition-all duration-300 hover:shadow-lg backdrop-blur-sm">
                          ...
                          <div className="text-right">
                            <p
                              className={`text-lg sm:text-xl font-bold ml-2 ${(marketData?.weather?.efficiency || 0) <= 0 ? "text-gray-400" : "text-emerald-300"}`}
                              data-testid="text-solar-efficiency">
                              {marketData?.weather?.efficiency !== undefined
                                ? `${Math.round(marketData.weather.efficiency)}%`
                                : "No data"}
                            </p>
                  ...
                  <Card className="p-6">
                    ...
                    <p className="text-center text-muted-foreground">
                      Real-time market data will appear here when households are
                      connected and trading energy.
                    </p>
                    {!user && (
                      <div className="mt-4 p-3 bg-blue-500/10 dark:bg-blue-950/50 rounded-lg">
                        <p className="text-sm text-blue-400 dark:text-blue-300 text-center">
                          Please log in to view authentic energy market data and
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  <Card className="p-4 sm:p-6 bg-gradient-to-br from-indigo-950/40 via-indigo-900/25 to-indigo-800/30 border border-indigo-500/20 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-base sm:text-lg font-semibold flex items-center gap-2">
                        ...
                        <span className="sm:hidden text-indigo-200">
                          Households
                        </span>
                      </h3>
                    ...
                    <p
                      className="text-2xl sm:text-3xl font-bold text-indigo-300 mb-2"
                      data-testid="text-total-households">
                      {networkAnalytics?.network?.totalHouseholds || 0}
                    </p>
                    <p className="text-xs sm:text-sm text-indigo-400">
                      {networkAnalytics?.network?.activeHouseholds || 0}{" "}
                      currently active
                    </p>
                  </Card>
                  ...
                  <Card className="p-4 sm:p-6 bg-gradient-to-br from-emerald-950/40 via-emerald-900/25 to-emerald-800/30 border border-emerald-500/20 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300">
                    ...
                    <p
                      className="text-2xl sm:text-3xl font-bold text-emerald-300 mb-2"
                      data-testid="text-battery-capacity">
                      {userHouseholds.length > 0
                        ? `${userHouseholds[0].batteryCapacity || 0} kWh`
                        : "0 kWh"}
                    </p>
                  ...
                  <Card className="p-4 sm:p-6 bg-gradient-to-br from-cyan-950/40 via-cyan-900/25 to-cyan-800/30 border border-cyan-500/20 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-base sm:text-lg font-semibold flex items-center gap-2">
                        ...
                        <span className="sm:hidden text-cyan-200">
                          Efficiency
                        </span>
                      </h3>
                    ...
                    <p
                      className="text-2xl sm:text-3xl font-bold text-cyan-300 mb-2"
                      data-testid="text-network-efficiency">
                      {networkAnalytics?.efficiency?.networkEfficiency || "0%"}
                    </p>
                    <p className="text-xs sm:text-sm text-cyan-400">
                      Avg. distance:{" "}
                      {networkAnalytics?.efficiency?.averageDistance || "0 km"}
                    </p>
                  </Card>
                </div>
                ...
                <Card className="p-6 bg-gradient-to-br from-slate-900/60 via-blue-950/40 to-slate-800/60 border border-blue-500/20 backdrop-blur-sm">
                  <h3 className="text-xl font-semibold mb-6 bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
                    Getting Started with SolarSense
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-gradient-to-r from-blue-900/40 to-blue-800/30 rounded-xl border border-blue-500/20 hover:border-blue-400/30 transition-all duration-300">
                      ...
                      <p className="text-sm text-slate-300">
                        Add your solar installation and battery details to join
                        the energy trading network.
                      </p>
                    </div>
                    <div className="p-4 bg-gradient-to-r from-emerald-900/40 to-emerald-800/30 rounded-xl border border-emerald-500/20 hover:border-emerald-400/30 transition-all duration-300">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <h4 className="font-semibold text-emerald-200">
                          2. Monitor Energy Flow
                        </h4>
                      </div>
                      <p className="text-sm text-slate-300">
                        Track your energy production, consumption, and battery
                        levels in real-time.
                      </p>
                    </div>
                    <div className="p-4 bg-gradient-to-r from-purple-900/40 to-purple-800/30 rounded-xl border border-purple-500/20 hover:border-purple-400/30 transition-all duration-300">
                      ...
                      <p className="text-sm text-slate-300">
                        Buy and sell surplus energy with neighbors for optimal
                        grid balance.
                      </p>
                    </div>
                    <div className="p-4 bg-gradient-to-r from-violet-900/40 to-violet-800/30 rounded-xl border border-violet-500/20 hover:border-violet-400/30 transition-all duration-300">
                      ...
                      <p className="text-sm text-slate-300">
                        Let our AI optimize energy distribution and trading
                  ...
                  <div className="flex flex-col lg:flex-row justify-between items-start gap-4 lg:gap-0">
                    <div className="flex-1">
                      ...
                      <p className="text-sm sm:text-base text-slate-200 mb-3 sm:mb-4">
                        Connect directly with neighbors to trade renewable
                        energy. Smart matching, instant transfers, fair pricing.
                      </p>
                      ...
                      <div className="grid grid-cols-3 gap-3 sm:gap-4 lg:gap-6 mt-4 sm:mt-6">
                        <div className="text-center p-3 sm:p-4 bg-gradient-to-br from-blue-900/40 to-blue-800/30 rounded-xl border border-blue-500/20">
                          <p className="text-lg sm:text-xl lg:text-2xl font-bold text-blue-300">
                            {energyOffers.length}
                          </p>
                        ...
                        <div className="text-center p-3 sm:p-4 bg-gradient-to-br from-purple-900/40 to-purple-800/30 rounded-xl border border-purple-500/20">
                          <p className="text-lg sm:text-xl lg:text-2xl font-bold text-purple-300">
                            {networkAnalytics?.trading?.averagePrice || "₹0"}
                          </p>
                    ...
                    <div className="flex flex-wrap gap-2 w-full lg:w-auto lg:flex-col">
                      <Button
                        onClick={() => setShowCreateTradeDialog(true)}
                        data-testid="button-create-trade"
                        className="bg-blue-600 hover:bg-blue-700 text-white font-medium shadow-lg text-xs sm:text-sm px-3 py-2">
                      ...
                      {tradesLoading ? (
                        <div className="space-y-3">
                          {[1, 2, 3].map((i) => (
                            <div
                              key={i}
                              className="p-4 border border-slate-600/30 rounded-lg animate-pulse bg-slate-800/40">
                        ...
                        energyOffers.map((item: any) => (
                          <div
                            key={item.trade.id}
                            className="p-3 sm:p-4 bg-gradient-to-r from-slate-800/50 to-slate-700/30 rounded-xl border border-emerald-500/30 hover:border-emerald-400/50 transition-all duration-300 hover:shadow-xl backdrop-blur-sm">
                            <div className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:gap-0">
                              <div className="flex-1 w-full sm:w-auto">
                                <div className="flex flex-wrap items-center gap-2 mb-2">
                                  <div className="w-3 h-3 bg-emerald-500/100 rounded-full animate-pulse"></div>
                                  <p className="font-bold text-emerald-400 text-base sm:text-lg">
                                    {item.trade.energyAmount} kWh
                                  </p>
                                  {(() => {
                                    const smartStatus = getSmartTradeStatus(
                                      item.trade.status,
                                      (item as any).acceptanceCount,
                                    );
                                    return (
                                      <span
                                        className={`${smartStatus.className} px-2 py-1 rounded-full text-xs font-medium`}>
                                        {smartStatus.text}
                                      </span>
                                    );
                                  })()}
                                  {(() => {
                                    const statusInfo = getApplicationStatusText(
                                      item.trade.id,
                                      (item as any).acceptanceCount,
                                    );
                                    return (
                                      <span
                                        className={`${statusInfo.className} px-2 py-1 rounded-full text-xs font-medium`}>
                                        {statusInfo.text}
                                      </span>
                                    );
                                  })()}
                              ...
                              <div className="w-full sm:w-auto sm:text-right">
                                <div className="bg-emerald-500/10 p-2 sm:p-3 rounded-lg mb-3">
                                  ...
                                  <p className="font-bold text-xl sm:text-2xl text-emerald-400">
                                    {formatTradeTotal(
                                      item.trade.energyAmount,
                                      item.trade.pricePerKwh,
                                    )}
                                  </p>
                                  <p className="text-xs text-muted-foreground">
                                    Price per kWh:{" "}
                                    {formatTradePrice(item.trade.pricePerKwh)}
                                  </p>
                                </div>
                                {isOwnTrade(item.trade) ? (
                                  ...
                                  <Button
                                    size="sm"
                                    onClick={() =>
                                      handleCancelApplication(item.trade.id)
                                    }
                        ...
                        <div className="text-center py-8">
                          ...
                          <h4 className="font-semibold text-primary-custom mb-2">
                            No Energy Offers Yet
                          </h4>
                          <p className="text-sm text-muted-foreground mb-4">
                            Be the first to sell surplus solar energy!
                          </p>
                          <Button
                            onClick={() => {
                      ...
                      {tradesLoading ? (
                        ...
                        energyRequests.map(
                          (request: {
                            ...
                            <div
                              key={request.trade.id}
                              className="p-3 sm:p-4 border-2 border-blue-500/30 rounded-xl hover:border-blue-400/50 transition-all duration-200 hover:shadow-md bg-gradient-to-r from-slate-800/50 to-slate-700/30 backdrop-blur-sm">
                              <div className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:gap-0">
                                <div className="flex-1 w-full sm:w-auto">
                                  <div className="flex flex-wrap items-center gap-2 mb-2">
                                    ...
                                    <p className="font-bold text-blue-400 text-base sm:text-lg">
                                      {request.trade.energyAmount} kWh
                                    </p>
                                    {(() => {
                                      const smartStatus = getSmartTradeStatus(
                                        request.trade.status,
                                        (request as any).acceptanceCount,
                                      );
                                      return (
                                        <span
                                          className={`${smartStatus.className} px-2 py-1 rounded-full text-xs font-medium`}>
                                ...
                                <div className="w-full sm:w-auto sm:text-right">
                                  <div className="bg-blue-500/10 p-2 sm:p-3 rounded-lg mb-3">
                                    ...
                                    <p className="font-bold text-xl sm:text-2xl text-blue-400">
                                      {formatTradeTotal(
                                        request.trade.energyAmount,
                                        request.trade.pricePerKwh,
                                      )}
                                    </p>
                                  ...
                                  {isOwnTrade(request.trade) ? (
                                    <div className="space-y-2">
                                      ...
                                      <Button
                                        ...
                                        variant="outline"
                                        onClick={() =>
                                          handleDeleteTrade(request.trade)
                                        }
                                        disabled={deleteTradeMutation.isPending}
                                        className="w-full border-red-300 text-red-700"
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-sm text-blue-300 mb-4">
                            Be the first to request clean energy!
                          </p>
                          <Button
                            onClick={() => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <Card className="p-3 sm:p-4 lg:p-6 bg-gradient-to-br from-purple-950/40 via-purple-900/25 to-pink-900/30 border border-purple-500/20 backdrop-blur-sm shadow-xl">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center p-3 bg-gradient-to-r from-slate-800/50 to-slate-700/30 rounded-xl border border-slate-600/30 hover:border-slate-500/50 transition-all duration-300">
                        ...
                        <span
                          className={`text-sm font-semibold ${(marketData?.gridStability || 0) > 70 ? "text-emerald-300" : (marketData?.gridStability || 0) > 40 ? "text-yellow-400" : "text-red-400"}`}>
                  ...
                  <Card className="p-3 sm:p-4 lg:p-6 bg-gradient-to-br from-slate-950/40 via-blue-950/25 to-slate-900/30 border border-blue-500/20 backdrop-blur-sm shadow-xl">
                    <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 flex items-center gap-2">
                      ...
                      <span className="text-sm sm:text-base bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
                        How Trading Works
                      </span>
                    </h3>
                    <div className="space-y-3 sm:space-y-4">
                      <div className="flex items-start gap-2 sm:gap-3 p-3 bg-gradient-to-r from-emerald-900/30 to-emerald-800/20 rounded-xl border border-emerald-500/20 hover:border-emerald-400/40 transition-all duration-300">
                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-emerald-500/20 rounded-full flex items-center justify-center text-emerald-400 font-bold text-xs sm:text-sm flex-shrink-0">
                          1
                        </div>
                        <div>
                          <h4 className="font-semibold text-xs sm:text-sm text-emerald-200">
                            Create Your Offer
                          </h4>
                          <p className="text-xs text-emerald-400">
                            Set your energy amount and price
                          </p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2 sm:gap-3 p-3 bg-gradient-to-r from-blue-900/30 to-blue-800/20 rounded-xl border border-blue-500/20 hover:border-blue-400/40 transition-all duration-300">
                        ...
                        <div>
                          <h4 className="font-semibold text-xs sm:text-sm text-blue-200">
                            Smart Matching
                          </h4>
                          <p className="text-xs text-blue-400">
                            AI finds the best local matches
                          </p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-gradient-to-r from-purple-900/30 to-purple-800/20 rounded-xl border border-purple-500/20 hover:border-purple-400/40 transition-all duration-300">
                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-purple-500/20 rounded-full flex items-center justify-center text-purple-400 font-bold text-xs sm:text-sm flex-shrink-0">
                          3
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-purple-400">
                            Automatic grid routing and billing
                          </p>
                        </div>
                      </div>
                    </div>
      ...
      {/* Create Trade Dialog */}
      <Dialog
        open={showCreateTradeDialog}
        onOpenChange={setShowCreateTradeDialog}>
            ...
            <form
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    form.setValue("pricePerKwh", currentMarketRate)
                  }
              ...
              {/* Total Display */}
              {form.watch("energyAmount") > 0 &&
                form.watch("pricePerKwh") > 0 && (
      ...
      <Dialog
        open={showAcceptTradeDialog}
        onOpenChange={setShowAcceptTradeDialog}>
            ...
            <div className="space-y-4 sm:space-y-6 pt-4">
              <div className="bg-gradient-to-r from-emerald-950/40 to-emerald-900/30 p-3 sm:p-4 rounded-lg border border-emerald-500/20">
                <h4 className="font-semibold text-emerald-200 mb-3 text-sm sm:text-base">
                  {acceptingTrade.trade.tradeType === "sell"
                    ? "🌟 Accept Energy Offer"
                    : "⚡ Fulfill Energy Request"}
                </h4>
                <div className="space-y-2 text-xs sm:text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">Energy Amount:</span>
                    <span className="font-semibold text-slate-200">
                      {acceptingTrade.trade.energyAmount} kWh
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">Price per kWh:</span>
                    <span className="font-semibold text-slate-200">
                      {formatTradePrice(acceptingTrade.trade.pricePerKwh)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center border-t border-emerald-500/20 pt-2">
                    ...
                    <span className="font-bold text-base sm:text-lg text-emerald-400">
                      {formatTradeTotal(
                        acceptingTrade.trade.energyAmount,
                        acceptingTrade.trade.pricePerKwh,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between items-start">
                    ...
                    <span className="font-medium text-right ml-2 break-words text-slate-200">
                      {acceptingTrade.trade.tradeType === "sell"
                        ? getHouseholdName(acceptingTrade, "seller")
                        : getHouseholdName(acceptingTrade, "buyer")}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-800/50 p-3 sm:p-4 rounded-lg border border-slate-600/50">
                ...
                <div className="space-y-2 text-xs sm:text-sm text-slate-300">
                  <div className="flex items-start gap-2">
                    ...
                    <span>
                      Your contact information will be shared with the other
                      party
                    </span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>
                      You'll receive their contact details for energy transfer
                      coordination
                    </span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>
                      SolarSense facilitates secure peer-to-peer energy exchange
                    </span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>
                      Trade completion is tracked for network transparency
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4 pt-4">
              <div className="p-4 bg-gray-50 rounded-lg">
                <h4 className="font-semibold mb-2">
                  {selectedTradeForDetails.trade.tradeType === "sell"
                    ? "Energy Offer"
                    : "Energy Request"}
                </h4>
                <p>
                  <strong>Amount:</strong>{" "}
                  {selectedTradeForDetails.trade.energyAmount} kWh
                </p>
                <p>
                  <strong>Price:</strong>{" "}
                  {formatTradePrice(selectedTradeForDetails.trade.pricePerKwh)}
                  /kWh
                </p>
                <p>
                  <strong>Total Value:</strong>{" "}
                  {formatTradeTotal(
                    selectedTradeForDetails.trade.energyAmount,
                    selectedTradeForDetails.trade.pricePerKwh,
                  )}
                </p>
                <p>
                  <strong>Created:</strong>{" "}
                  {new Date(
                    selectedTradeForDetails.trade.createdAt,
                  ).toLocaleString()}
                </p>
              </div>
              ...
              <div className="p-4 bg-blue-500/10 rounded-lg">
                ...
                <p className="text-sm text-gray-600 mb-3">
                  Click below to send an automated email expressing your
                  interest in this trade. Your contact details will be shared
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    toast({
                      title: "Email Opened",
                      description:
                        "Your email client has been opened with a pre-filled message. Send it to express your interest!",
                    });
              ...
              <div className="p-4 bg-yellow-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  <strong>Area:</strong> Ludhiana, Punjab
                  <br />
                  <strong>Household:</strong>{" "}
                  {selectedTradeForDetails.trade?.tradeType === "sell"
                    ? getHouseholdName(selectedTradeForDetails, "seller")
                    : getHouseholdName(selectedTradeForDetails, "buyer")}
                  <br />
                  <strong>Contact:</strong> Details shared upon mutual interest
                  <br />
                  <strong>Safety:</strong> All trades are verified through
                  SolarSense platform
                </p>
              </div>
            </div>
            ...
            <form
              ...
              <div className="bg-blue-500/10 p-4 rounded-lg border border-blue-200">
                ...
                <p className="text-sm text-blue-400">
                  Market rate: <strong>₹5/kWh</strong> • Weather impact:{" "}
                  <strong>Stormy conditions reducing solar</strong>
                </p>
              </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {field.value > 0 && (
                        <div className="text-xs text-gray-600 bg-gray-50 p-2 rounded">
                          💰 Total Value: ₹
                          {Math.round(
                            (editForm.watch("energyAmount") || 0) * field.value,
                          )}
                        </div>
                      )}
                    </div>
