...
import {
  insertEnergyTradeSchema,
  insertHouseholdSchema,
  insertChatMessageSchema,
  insertTradeAcceptanceSchema,
  simulationWeatherSchema,
  simulationOutageSchema,
} from "@shared/schema";
...
export function setupRoutes(app: Express) {
    ...
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.status(401).json({
          error:
            "Authentication required. Please log in to view energy trades.",
        });
      }
  ...
  app.post("/api/energy-trades", async (req, res) => {
    ...
    try {
      ...
      if (!energyAmount || !pricePerKwh || !tradeType) {
        return res.status(400).json({
          error:
            "Missing required fields: energyAmount, pricePerKwh, tradeType",
        });
      }
      ...
      if (energyAmount <= 0 || pricePerKwh <= 0) {
        return res
          .status(400)
          .json({ error: "Energy amount and price must be positive" });
      }

      // Validate household ownership
      const userHouseholdIds = userHouseholds.map((h) => h.id);
      const requestedHouseholdId =
        tradeType === "sell" ? sellerHouseholdId : buyerHouseholdId;

      if (
        requestedHouseholdId &&
        !userHouseholdIds.includes(requestedHouseholdId)
      ) {
      ...
      const tradeData = {
        sellerHouseholdId:
          tradeType === "sell"
            ? sellerHouseholdId || userHouseholds[0].id
            : undefined,
        buyerHouseholdId:
          tradeType === "buy"
            ? buyerHouseholdId || userHouseholds[0].id
            : undefined,
  ...
  app.put("/api/energy-trades/:id", async (req, res) => {
    ...
    try {
      ...
      const ownsThisTrade =
        (existingTrade.sellerHouseholdId &&
          userHouseholdIds.includes(existingTrade.sellerHouseholdId)) ||
        (existingTrade.buyerHouseholdId &&
          userHouseholdIds.includes(existingTrade.buyerHouseholdId));

      if (!ownsThisTrade) {
        return res
          .status(403)
          .json({ error: "You can only edit your own trades" });
      }
      ...
      if (applications && applications.length > 0) {
        return res.status(409).json({
          error:
            "Cannot edit trade after users have applied. You can award the trade to an applicant instead.",
        });
      }
  ...
  app.patch("/api/energy-trades/:id/cancel", async (req, res) => {
    ...
    try {
      ...
      if (existingTrade.status !== "pending") {
        return res
          .status(400)
          .json({ error: "Only pending trades can be cancelled" });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ error: "You can only cancel your own trades" });
      }
      ...
      try {
        ...
        if (applications.length > 0) {
          console.log(
            `📧 Notifying ${applications.length} applicants about trade cancellation`,
          );
      ...
      res.json({
        ...
        message:
          "Trade cancelled successfully. All applicants have been notified.",
      });
    } catch (error) {
  ...
  app.delete("/api/energy-trades/:id", async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ error: "You can only delete your own trades" });
      }
      ...
      try {
        ...
        if (applications.length > 0) {
          console.log(
            `📧 Notifying ${applications.length} applicants about trade deletion`,
          );
      ...
      res.json({
        success: true,
        message:
          "Trade deleted successfully. All applicants have been notified.",
      });
    } catch (error) {
  ...
  app.get("/api/energy-readings", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.status(401).json({
          error:
            "Authentication required. Please log in to view energy readings.",
        });
      }
  ...
  app.get("/api/market/realtime", async (req, res) => {
    try {
      ...
      if (!latitude || !longitude) {
        return res.status(400).json({
          error:
            "Location required for accurate market data. Please provide latitude and longitude.",
        });
      }
  ...
  app.get("/api/analytics/network", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.status(401).json({
          error:
            "Authentication required. Please log in to view network analytics.",
        });
      }
  ...
  app.post("/api/ai/chat", async (req, res) => {
    try {
      ...
      try {
        // Get user context
        const households = req.user
          ? await storage.getHouseholdsByUser(req.user.id)
          : [];
        const userContext = {
          username,
        ...
        const errorResponse =
          "Energy assistant is temporarily unavailable. Please try again later.";
      ...
      const analytics = {
        ...
        equity: {
          ...
          vulnerableHouseholds:
            typeof equityData.vulnerableHouseholds === "number"
              ? equityData.vulnerableHouseholds
              : equityData.vulnerableHouseholds.length,
  ...
  app.post("/api/trade-acceptances", async (req, res) => {
    ...
    try {
      ...
      if (trade.status !== "pending") {
        return res
          .status(400)
          .json({ error: "This trade is no longer accepting applications" });
      }
      ...
      if (isOwnTrade) {
        return res
          .status(400)
          .json({ error: "You cannot apply to your own trade" });
      }

      // Check for duplicate applications
      const existingApplications =
        await storage.getTradeAcceptancesByTrade(tradeId);
      const alreadyApplied = existingApplications.some(
        (app) => app.acceptorHouseholdId === acceptorHouseholdId,
      );

      if (alreadyApplied) {
        return res
          .status(400)
          .json({ error: "You have already applied to this trade" });
      }
      ...
      console.log(
        `📝 Application created for trade ${validation.data.tradeId} by user ${req.user!.id} (household ${acceptorHouseholdId})`,
      );
      ...
      try {
        ...
        if (trade) {
          ...
          if (offerCreatorUser && household) {
            ...
            console.log(
              `📧 Email notification sent to offer creator: ${offerCreatorUser.email}`,
            );
          }
        }
      } catch (emailError) {
      ...
      res.json({
        ...
        acceptance,
        message:
          "Application submitted! The offer creator has been notified. They will review all applications and choose who to share their energy with.",
      });
    } catch (error) {
  ...
  app.get("/api/energy-trades/:id/applications", async (req, res) => {
    ...
    try {
      ...
      if (!isOwner) {
        return res
          .status(403)
          .json({ error: "Only trade owner can view applications" });
      }
      ...
      const enrichedApplications = await Promise.all(
        applications.map(async (app) => {
          const applicantHousehold = await storage.getHousehold(
            app.acceptorHouseholdId,
          );
          const applicantUser = applicantHousehold
            ? await storage.getUser(applicantHousehold.userId)
            : null;
  ...
  app.post("/api/energy-trades/:id/award", async (req, res) => {
    ...
    try {
      ...
      if (!isOwner) {
        return res
          .status(403)
          .json({ error: "Only trade owner can award trades" });
      }
      ...
      await Promise.all(
        otherApplications.map((app) =>
          storage.updateTradeAcceptanceStatus(app.id, "declined"),
        ),
      );

      console.log(
        `🏆 Trade ${tradeId} awarded to application ${applicationId} (household ${acceptorHouseholdId})`,
      );
      ...
      try {
        ...
        if (acceptorUser) {
          ...
          console.log(
            `📧 Contact details sent to acceptor: ${acceptorUser.email}`,
          );
          ...
          console.log(
            `📧 Contact details sent to offer creator: ${offerCreator.email}`,
          );
          ...
          console.log(
            `📱 Application status updated to 'contacted' for coordination phase`,
          );
        }
      } catch (emailError) {
        console.error(
      ...
      res.json({
        success: true,
        message:
          "Trade awarded successfully! Contact information has been automatically shared between both parties via email.",
  ...
  app.post("/api/trade-acceptances/:id/share-contact", async (req, res) => {
    ...
    try {
      ...
      try {
        if (contactInfo.contactInfo && contactInfo.trade) {
          ...
          if (recipientUser) {
            ...
            console.log(
              `📧 Contact sharing notification sent to: ${recipientUser.email}`,
            );
          }
        }
      } catch (emailError) {
      ...
      res.json({
        success: true,
        message:
          "Contact information shared for energy delivery coordination. The other party has been notified via email.",
  ...
  app.delete("/api/trade-acceptances/:id", async (req, res) => {
    ...
    try {
      ...
      if (!acceptance) {
        return res
          .status(404)
          .json({ error: "Trade acceptance not found or access denied" });
      }
      ...
      try {
        if (offerCreator && trade) {
          ...
          console.log(
            `📧 Application cancellation notification sent to offer creator: ${offerCreator.email}`,
          );
        }
      } catch (emailError) {
        console.error(
      ...
      res.json({
        success: true,
        message:
          "Trade application cancelled successfully. The offer creator has been notified.",
      });
    } catch (error) {
    ...
    try {
      ...
      if (!acceptance) {
        ...
        for (const trade of userTrades) {
          const tradeAcceptances = await storage.getTradeAcceptancesByTrade(
            trade.id,
          );
          const foundAcceptance = tradeAcceptances.find(
            (a) => a.id === acceptanceId,
          );
          if (foundAcceptance) {
      ...
      if (!ownsThisTrade) {
        return res
          .status(403)
          .json({ error: "You can only make decisions on your own trades" });
      }
      ...
      if (decision === "accept") {
        ...
        res.json({
          ...
          acceptance: updatedAcceptance,
          message:
            "Application accepted! You can now share contact information for energy delivery coordination.",
        });
      } else {
        ...
        res.json({
          ...
          acceptance: updatedAcceptance,
          message:
            "Application rejected. Trade remains available for other applicants.",
        });
      }
    } catch (error) {
  ...
  app.patch("/api/trade-acceptances/:id/applicant-reject", async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        acceptance: updatedAcceptance,
        message:
          "Application rejected. Trade is now available for the owner to choose another applicant.",
      });
    } catch (error) {
