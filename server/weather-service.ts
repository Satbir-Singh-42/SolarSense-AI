...
class WeatherService {
  ...
  private generateFallbackWeather(
    ...
    const seasonalBaseTemp = [5, 8, 15, 22, 28, 32, 35, 33, 28, 20, 12, 7][
      month
    ];

    // Daily temperature variation (warmer in afternoon)
    const hourlyVariation = isDay
      ? Math.sin(((hour - 6) / 12) * Math.PI) * 8
      : 0;
  ...
  calculateSolarEfficiency(
    ...
    const timeInDay =
      (currentTime - weather.sunrise) / (weather.sunset - weather.sunrise);
    ...
    const tempImpact =
      weather.temperature <= 25
        ? 1.0
        : Math.max(0.7, 1 - (weather.temperature - 25) * 0.004);

    return Math.max(
      0,
      conditionMultipliers[weather.condition] *
        sunAngleMultiplier *
        cloudImpact *
        tempImpact,
    );
