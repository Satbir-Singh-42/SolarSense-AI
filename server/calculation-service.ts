...
const EFFICIENCY_FACTORS: EfficiencyFactors = {
  orientation: {
    ...
    southeast: 0.9, // Very good
    southwest: 0.9, // Very good
    east: 0.85, // Good
    west: 0.85, // Good
    northeast: 0.75, // Fair
    northwest: 0.75, // Fair
...
function calculatePanelArea(panelCount: number): number {
  return panelCount * MARKET_STANDARDS.panelArea;
}

function calculateCoverage(panelCount: number, roofArea: number): number {
  return (calculatePanelArea(panelCount) / roofArea) * 100;
}

function calculatePowerOutput(panelCount: number): number {
...
function calculateEfficiencyScore(
  ...
  const orientationFactor =
    EFFICIENCY_FACTORS.orientation[orientation.toLowerCase()] || 0.85;
  const pitchFactor = EFFICIENCY_FACTORS.pitch[pitch.toLowerCase()] || 0.9;
  const shadingFactor =
    EFFICIENCY_FACTORS.shading[shading.toLowerCase()] || 0.9;
  const weatherFactor =
    EFFICIENCY_FACTORS.weatherConditions[weatherConditions.toLowerCase()] ||
    0.95;

  return Math.round(
    orientationFactor * pitchFactor * shadingFactor * weatherFactor * 100,
  );
}
...
function calculateInstallationCost(panelCount: number): number {
