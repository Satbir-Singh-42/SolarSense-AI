...
__export(schema_exports, {
  ...
  simulationOutageSchema: () => simulationOutageSchema,
  simulationWeatherSchema: () => simulationWeatherSchema,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    signupSchema = z.object({
      ...
      batteryCapacity: z.coerce.number().int("Battery capacity must be a whole number").min(1, "Battery capacity must be at least 1 kWh"),
...
var MemStorage = class {
  ...
  async deleteTradeAcceptance(id) {
...
var DatabaseStorage = class {
  ...
  async getEnergyTradesByUser(userId, limit = 50) {
    ...
    const trades = await db2.select().from(energyTrades).where(
      and(
        ...
        // Only return active trades that user can manage
      )
    ).orderBy(desc(energyTrades.createdAt)).limit(limit);
    return trades.map((trade) => ({
      ...trade,
  ...
  async getNetworkAnalytics() {
    ...
    return {
      ...
      trading: {
        totalTrades,
        averagePrice: avgPriceNum > 0 ? `\u20B9${avgPriceNum}` : "\u20B90",
        // Format as currency string
        carbonSaved: this.calculateCarbonSaved(todaysTrades)
  ...
  calculateNetworkEfficiency(totalGeneration, currentBatteryStorage, totalTrades) {
    console.log(`\u{1F4CA} Network efficiency calculation:
      totalGeneration: ${totalGeneration} W
      currentBatteryStorage: ${currentBatteryStorage} kWh  
      totalTrades: ${totalTrades}`);
    const households2 = totalGeneration > 0 ? 8 : 0;
  ...
  async getTradeAcceptancesByUser(userId) {
    const db2 = await this.getDb();
    const acceptancesWithTrades = await db2.select({
    ...
    }).from(tradeAcceptances).leftJoin(energyTrades, eq(tradeAcceptances.tradeId, energyTrades.id)).where(eq(tradeAcceptances.acceptorUserId, userId));
    const enrichedAcceptances = await Promise.all(
      acceptancesWithTrades.map(async (acceptance) => {
        ...
        if (ownerHouseholdId) {
          const householdResult = await db2.select({
            household: {
              id: households.id,
              name: households.name,
              address: households.address
          ...
          }).from(households).leftJoin(users, eq(households.userId, users.id)).where(eq(households.id, ownerHouseholdId)).limit(1);
  ...
  async deleteTradeAcceptance(id) {
    const db2 = await this.getDb();
    const result = await db2.delete(tradeAcceptances).where(eq(tradeAcceptances.id, id));
    return result.rowCount ? result.rowCount > 0 : false;
  }
  // Helper method to calculate carbon saved from trades
  calculateCarbonSaved(trades) {
    ...
    if (carbonSavedKg < 1) {
      return `${Math.round(carbonSavedKg * 1e3)} g`;
    }
  ...
  async getAvailableOffersForUser(userId) {
    ...
    const availableOffers = await db2.select({
      ...
      households,
      or(
        eq(energyTrades.sellerHouseholdId, households.id),
        // For sell trades
        eq(energyTrades.buyerHouseholdId, households.id)
        // For buy trades
      )
    ).leftJoin(users, eq(households.userId, users.id)).where(
      and(
        eq(energyTrades.status, "pending"),
    ...
    const tradeIds = availableOffers.map((offer) => offer.trade.id);
    const acceptanceCounts = tradeIds.length > 0 ? await db2.select({
      tradeId: tradeAcceptances.tradeId,
      count: sql`count(*)`
    }).from(tradeAcceptances).where(and(
...
var MLEnergyEngine = class {
  ...
  historicalData = new HistoricalEnergyData();
  neuralPredictor = new SimpleNeuralNetwork();
  adaptiveLearning = new AdaptiveLearningModel();
  accuracyMetrics = new AccuracyTracker();
  ...
  predictEnergyGeneration(household, weather, timeOfDay) {
    ...
    const neuralInputs = this.normalizeGenerationInputs([
      ...
      this.getSeasonFromTime(timeOfDay),
      // Use provided time, not system time
      this.getDayTypeFromTime(timeOfDay)
      // Use provided time, not system time
    ]);
  ...
  predictEnergyDemand(household, timeOfDay, dayOfWeek) {
    ...
    const demandInputs = this.normalizeDemandInputs([
      ...
      baseDemand,
      timeOfDay < 6 || timeOfDay > 22 ? 0.7 : 1
      // night factor from provided time
    ]);
  ...
  calculateSmartVariance(household, timeOfDay, dayOfWeek) {
    let variance = 1;
  ...
  normalizeGenerationInputs(inputs) {
    return [
      Math.max(-1, Math.min(1, (inputs[0] - 25) / 25)),
      // temperature (0-50°C range)
      Math.max(-1, Math.min(1, (inputs[1] - 50) / 50)),
      // cloud cover (0-100% range)
      Math.max(-1, Math.min(1, (inputs[2] - 15) / 15)),
      // wind speed (0-30 km/h range)
      Math.max(-1, Math.min(1, (inputs[3] - 12) / 12)),
      // time of day (0-24 range)
      Math.max(-1, Math.min(1, inputs[4] / 20)),
      // solar capacity (0-20kW typical range)
      Math.max(-1, Math.min(1, inputs[5] - 1)),
      // trend factor (0-2 range)
      Math.max(-1, Math.min(1, inputs[6] / 6)),
      // season (0-12 months)
      Math.max(-1, Math.min(1, inputs[7] / 3))
      // day type (0-6 days)
    ];
  }
  normalizeDemandInputs(inputs) {
    return [
      Math.max(-1, Math.min(1, (inputs[0] - 12) / 12)),
      // time of day (0-24 range)
      Math.max(-1, Math.min(1, (inputs[1] - 3) / 3)),
      // day of week (0-6 range)
      Math.max(-1, Math.min(1, inputs[2] / 6)),
      // season (0-12 months)
      Math.max(-1, Math.min(1, inputs[3] / 50)),
      // battery capacity (0-50kWh typical)
      Math.max(-1, Math.min(1, (inputs[4] - 50) / 50)),
      // battery level (0-100%)
      Math.max(-1, Math.min(1, inputs[5] - 1)),
      // trend factor (0-2 range)
      Math.max(-1, Math.min(1, inputs[6] / 5)),
      // base demand (0-5kW typical)
      Math.max(-1, Math.min(1, inputs[7] * 2 - 1))
      // night factor (0-1 to -1-1)
    ];
  }
  getSeasonFromTime(timeOfDay) {
    const date = /* @__PURE__ */ new Date();
    return date.getMonth();
  }
  getDayTypeFromTime(timeOfDay) {
    const date = /* @__PURE__ */ new Date();
    return date.getDay();
  }
  generateGroundTruthGeneration(household, weather, timeOfDay) {
    const baseGeneration = household.solarCapacity || 0;
    ...
    const systemEfficiency = 0.95 + Math.random() * 0.08;
    return Math.max(0, physicsBasedGeneration * systemEfficiency);
  }
  generateGroundTruthDemand(household, timeOfDay, dayOfWeek) {
    const baseDemand = this.getBaseDemand(household);
    ...
    const occupancyVariation = 0.9 + Math.random() * 0.2;
  ...
  identifyTradingPairs(networkState) {
    const suppliers = networkState.households.filter((h) => {
      ...
      const hasBatteryCapacity = h.batteryLevel > Math.max(2, h.batteryCapacity * 0.4);
    ...
    const demanders = networkState.households.filter((h) => {
      ...
      const hasLowBattery = h.batteryLevel < Math.max(1.5, h.batteryCapacity * 0.3);
      const isOnline = h.isOnline !== false;
    ...
    sortedDemanders.forEach((demander) => {
      const demandNeed = Math.max(
        ...
        demander.batteryCapacity * 0.6 - demander.batteryLevel
        // Target 60% battery level
      );
      if (demandNeed < 0.3) return;
      const availableSuppliers = Array.from(supplierBalances.entries()).filter(([_, supply]) => supply >= 0.3).map(([supplierId, supply]) => ({
      ...
      if (bestMatch) {
        const energyAmount = Math.min(
          ...
          3
          // Increased max to 3kWh per trade for better efficiency
        );
        if (energyAmount >= 0.3) {
...
var HistoricalEnergyData = class {
  generationHistory = /* @__PURE__ */ new Map();
  demandHistory = /* @__PURE__ */ new Map();
  accuracyHistory = /* @__PURE__ */ new Map();
  weatherCorrelations = /* @__PURE__ */ new Map();
  // Store actual vs predicted data for learning
  recordActual(householdId, actual, predicted) {
  ...
  getPattern(householdId, type) {
    const key = `${type.substring(0, 3)}_${householdId}`;
    return type === "generation" ? this.generationHistory.get(key) || [] : this.demandHistory.get(key) || [];
  }
  getAccuracy(householdId) {
    const accuracies = this.accuracyHistory.get(`acc_${householdId}`) || [];
    return accuracies.length > 0 ? accuracies.reduce((sum, acc) => sum + acc, 0) / accuracies.length : 0.75;
  }
  // Advanced pattern recognition using moving averages and trend analysis
  getTrend(householdId, type) {
    ...
    return olderAvg === 0 ? 1 : recentAvg / olderAvg;
  }
};
var SimpleNeuralNetwork = class {
  weights = {
  ...
  predict(inputs) {
    ...
    const hiddenInputs = this.weights.input.map(
      (w, i) => w * (normalizedInputs[i] || 0)
    );
    const hiddenOutputs = hiddenInputs.map((x) => this.sigmoid(x));
    const finalInputs = this.weights.hidden.map(
      (w, i) => w * (hiddenOutputs[i] || 0)
    );
  ...
  normalizeInputs(inputs) {
    return inputs.map((input) => {
      if (input === 0) return 0;
      return Math.max(-1, Math.min(1, input / 100));
    });
  }
  randomWeights(size) {
...
var AdaptiveLearningModel = class {
  confidenceScores = /* @__PURE__ */ new Map();
  recentErrors = /* @__PURE__ */ new Map();
  adaptationFactors = /* @__PURE__ */ new Map();
  // Update model confidence based on prediction accuracy
  updateConfidence(householdId, actualValue, predictedValue) {
  ...
  getConfidence(householdId) {
    return this.confidenceScores.get(householdId) || 0.75;
  }
  getAdaptationFactor(householdId) {
    return this.adaptationFactors.get(householdId) || 1;
  }
  calculateAdaptationFactor(errors) {
    if (errors.length < 5) return 1;
...
var AccuracyTracker = class {
  predictions = /* @__PURE__ */ new Map();
  record(type, householdId, actual, predicted) {
    const key = `${type}_${householdId}`;
    if (!this.predictions.has(key)) {
      this.predictions.set(key, []);
    }
    const records = this.predictions.get(key);
    records.push({ actual, predicted, timestamp: /* @__PURE__ */ new Date() });
    if (records.length > 50) {
      records.shift();
    }
  }
  getAccuracy(type, householdId) {
    let allRecords = [];
    if (householdId) {
      const key = `${type}_${householdId}`;
...
var PriceOptimizer = class {
  calculateOptimalPrices(pairs, networkState) {
    ...
    pairs.forEach((pair) => {
      ...
      prices.set(pair.supplierId, finalPrice);
    });
    return prices;
  }
  getTimeOfUsePrice(hour) {
    if (hour >= 18 && hour <= 22) return 8;
    if (hour >= 6 && hour <= 9) return 6;
    if (hour >= 10 && hour <= 17) return 5;
    return 3;
  }
...
var SimulationEngine = class {
  ...
  calculateAverageDistance(households2) {
    if (households2.length < 2) return 1.2;
    const avgDistanceBetweenHouseholds = Math.sqrt(households2.length) * 0.3;
    return Math.max(0.5, Math.min(5, avgDistanceBetweenHouseholds));
  }
  calculateNetworkEfficiency(totalGeneration, totalConsumption) {
    if (totalGeneration === 0 && totalConsumption === 0) return 85;
    const supplyDemandRatio = totalConsumption > 0 ? totalGeneration / totalConsumption : 1;
...
// server/routes.ts
function getSimulationEngine(storage2) {
  if (!globalThis.__simulationEngine) {
    console.log("\u{1F504} Creating new SimulationEngine instance");
    globalThis.__simulationEngine = new SimulationEngine(storage2);
  } else {
...
function setupRoutes(app) {
  ...
  app.post("/api/test-email", async (req, res) => {
    ...
    try {
      ...
      console.log(`\u{1F4E7} Testing email delivery to: ${testEmail}`);
    ...
    try {
      ...
      const analyses = userTrades.map((trade) => ({
        ...
        results: {
          ...
          pricePerKwh: Math.round(trade.pricePerKwh),
          // Already in rupees
  ...
  app.post("/api/energy-trades", async (req, res) => {
    ...
    try {
      ...
      const tradeData = {
        ...
        pricePerKwh: Math.round(parseFloat(pricePerKwh)),
        // Store as rupees
        tradeType
      };
  ...
  app.post("/api/trade-acceptances", async (req, res) => {
    ...
    try {
      ...
      const userHouseholdIds = userHouseholds.map((h) => h.id);
      const isOwnTrade = trade.sellerHouseholdId && userHouseholdIds.includes(trade.sellerHouseholdId) || trade.buyerHouseholdId && userHouseholdIds.includes(trade.buyerHouseholdId);
      if (isOwnTrade) {
      ...
      try {
        ...
        if (trade2) {
          ...
          let household = null;
          if (trade2.sellerHouseholdId) {
            household = await storage.getHousehold(trade2.sellerHouseholdId);
            if (household) {
              offerCreatorUser = await storage.getUser(household.userId);
            }
          } else if (trade2.buyerHouseholdId) {
            household = await storage.getHousehold(trade2.buyerHouseholdId);
            if (household) {
              offerCreatorUser = await storage.getUser(household.userId);
            }
          }
  ...
  app.get("/api/energy-trades/:id/applications", async (req, res) => {
    ...
    try {
      ...
      const userHouseholds = await storage.getHouseholdsByUser(req.user.id);
      const userHouseholdIds = userHouseholds.map((h) => h.id);
      const isOwner = trade.sellerHouseholdId && userHouseholdIds.includes(trade.sellerHouseholdId) || trade.buyerHouseholdId && userHouseholdIds.includes(trade.buyerHouseholdId);
      if (!isOwner) {
  ...
  app.patch("/api/trade-acceptances/:id/owner-decision", async (req, res) => {
    ...
    try {
      ...
      const userHouseholds = await storage.getHouseholdsByUser(req.user.id);
      const userHouseholdIds = userHouseholds.map((h) => h.id);
      const ownsThisTrade = trade.sellerHouseholdId && userHouseholdIds.includes(trade.sellerHouseholdId) || trade.buyerHouseholdId && userHouseholdIds.includes(trade.buyerHouseholdId);
      if (!ownsThisTrade) {
