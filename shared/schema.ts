...
export const households = pgTable(
  "households",
  {
    ...
    currentBatteryLevel: integer("current_battery_percent")
      .notNull()
      .default(50),
...
export const energyTrades = pgTable(
  "energy_trades",
  {
    id: serial("id").primaryKey(),
...
export const tradeAcceptances = pgTable(
  "trade_acceptances",
  {
    ...
    tradeId: integer("trade_id")
      .notNull()
      .references(() => energyTrades.id, { onDelete: "cascade" }),
    acceptorUserId: integer("acceptor_user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    acceptorHouseholdId: integer("acceptor_household_id")
      .notNull()
      .references(() => households.id, { onDelete: "cascade" }),
...
export const energyTradesRelations = relations(
  energyTrades,
  ({ one, many }) => ({
...
export const tradeAcceptancesRelations = relations(
  tradeAcceptances,
  ({ one }) => ({
...
export const signupSchema = z
  .object({
    ...
    solarCapacity: z.coerce
      .number()
      .min(1000, "Solar capacity must be at least 1000 watts"),
...
export const insertEnergyReadingSchema = createInsertSchema(
  energyReadings,
).pick({
  householdId: true,
...
export const insertEnergyTradeSchema = createInsertSchema(energyTrades)
  .pick({
  ...
  .refine(
    ...
    {
      message:
        "Either seller or buyer household ID must be provided based on trade type",
    },
  );
...
export const insertTradeAcceptanceSchema = createInsertSchema(
  tradeAcceptances,
).pick({
  tradeId: true,
  acceptorUserId: true,
...
export const simulationWeatherSchema = z.object({
  condition: z.enum([
    "sunny",
    "partly-cloudy",
    "cloudy",
...
export const simulationOutageSchema = z.object({
  householdIds: z.array(z.number().int().positive()).optional().default([]),
});

export type SimulationWeatherRequest = z.infer<typeof simulationWeatherSchema>;
export type SimulationOutageRequest = z.infer<typeof simulationOutageSchema>;
